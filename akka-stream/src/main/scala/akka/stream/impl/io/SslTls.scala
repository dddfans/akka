...
object SslTlsCipherActor {
  ...
  def props(settings: ActorFlowMaterializerSettings,
            ...
            tracing: Boolean,
            role: StreamTls.Role,
            closing: StreamTls.Closing): Props =
...
class SslTlsCipherActor(settings: ActorFlowMaterializerSettings, sslContext: SSLContext,
                        firstSession: NegotiateNewSession, tracing: Boolean,
                        role: StreamTls.Role, closing: StreamTls.Closing)
  ...
  val engine: SSLEngine = {
    ...
    e.setUseClientMode(role == StreamTls.Client)
    e
  }
  ...
  override def receive = LoggingReceive(inputBunch.subreceive.orElse[Any, Unit](outputBunch.subreceive))
